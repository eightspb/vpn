...
Опции для полного деплоя:
  ...
  --with-xray           Задеплоить XRAY Reality на VPS1 (VLESS+Reality, порт 443)
    ...
    case "$mode" in
        ...
        full)
            ...
            if [[ "$with_xray" == "true" ]]; then
                log "Деплой XRAY Reality (deploy-xray.sh)..."
                bash "${SCRIPT_DIR}/scripts/deploy/deploy-xray.sh" "${extra_args[@]+"${extra_args[@]}"}"
            fi
            ;;
        vps1)
