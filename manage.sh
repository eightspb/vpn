...
#   bash manage.sh deploy                            # maintenance deploy VPN
...
Частые сценарии:
  ...
  bash manage.sh deploy                  # maintenance deploy VPN без ротации ключей
...
Режимы:
  ...
  (без флага)   Maintenance deploy: VPS1 + VPS2 без ротации WG-ключей.
                  Перед изменениями создаёт snapshot и при ошибке запускает rollback.
  --vps1        Только VPS1 (reprovision-режим: генерирует новые WG-ключи)
  --vps2        Только VPS2 (требует --keys-file от deploy-vps1)
  --split-tunneling  Включить раздельное туннелирование на VPS1
                       (RU TLD + category-ru идут напрямую, остальное через VPS2)
                       Клиентские конфиги НЕ изменяются.
...
Примеры:
  ...
  # Maintenance deploy VPN без ротации ключей:
  ...
  # Только VPS1 (reprovision-режим, генерирует новые WG-ключи):
...
cmd_deploy() {
    ...
    if [[ "${1:-}" == "--admin" ]]; then
        ...
        err "--proxy удалён: legacy proxy больше не поддерживается. Используйте AdGuard Home через обычный deploy."
    elif [[ "${1:-}" == "--split-tunneling" ]]; then
        mode="split-tunneling"; shift
    fi
    ...
    case "$mode" in
        admin)
            ...
            ;;
        full)
            local snapshot_script="${SCRIPT_DIR}/scripts/deploy/deploy-snapshot-rollback.sh"
            local snapshot_id
            local snapshot_args=()
            local deploy_rc=0
            local rollback_rc=0
            ...
            [[ -f "$snapshot_script" ]] || err "Не найден rollback snapshot script: $snapshot_script"
            snapshot_id="$(date +%Y%m%d-%H%M%S)"
            while [[ "$i" -lt "${#extra_args[@]}" ]]; do
                case "${extra_args[$i]}" in
                    --vps1-ip|--vps1-user|--vps1-key|--vps1-pass|--vps2-ip|--vps2-user|--vps2-key|--vps2-pass)
                        if [[ $((i + 1)) -ge "${#extra_args[@]}" ]]; then
                            err "${extra_args[$i]} requires a value"
                        fi
                        snapshot_args+=("${extra_args[$i]}" "${extra_args[$((i + 1))]}")
                        i=$((i + 2))
                        ;;
                    *)
                        i=$((i + 1))
            ...
            log "Создание pre-deploy snapshot для автоматического rollback: ${snapshot_id}"
            bash "$snapshot_script" snapshot --snapshot-id "$snapshot_id" "${snapshot_args[@]+"${snapshot_args[@]}"}"

            log "Запуск maintenance deploy (deploy.sh, без ротации существующих WG-ключей)..."
            set +e
            ...
            if [[ "$deploy_rc" -ne 0 ]]; then
                warn "Deploy failed with exit code ${deploy_rc}; запускаю автоматический rollback к snapshot ${snapshot_id}"
                set +e
                bash "$snapshot_script" rollback --snapshot-id "$snapshot_id" "${snapshot_args[@]+"${snapshot_args[@]}"}"
                rollback_rc=$?
                set -e
                if [[ "$rollback_rc" -eq 0 ]]; then
                    err "Deploy failed; rollback completed to snapshot ${snapshot_id}. Manual rollback command: bash scripts/deploy/deploy-snapshot-rollback.sh rollback --snapshot-id ${snapshot_id}"
                fi
                err "Deploy failed and automatic rollback failed with exit code ${rollback_rc}. Manual rollback command: bash scripts/deploy/deploy-snapshot-rollback.sh rollback --snapshot-id ${snapshot_id}"
            fi
            ...
            if [[ "$regen_configs" == "true" ]]; then
                ...
                set -e
                if [[ "$deploy_rc" -ne 0 ]]; then
                    warn "Перегенерация конфигов failed with exit code ${deploy_rc}; запускаю автоматический rollback к snapshot ${snapshot_id}"
                    set +e
                    bash "$snapshot_script" rollback --snapshot-id "$snapshot_id" "${snapshot_args[@]+"${snapshot_args[@]}"}"
                    rollback_rc=$?
                    set -e
                    if [[ "$rollback_rc" -eq 0 ]]; then
                        err "Config regeneration failed; rollback completed to snapshot ${snapshot_id}. Manual rollback command: bash scripts/deploy/deploy-snapshot-rollback.sh rollback --snapshot-id ${snapshot_id}"
                    fi
                    err "Config regeneration failed and automatic rollback failed with exit code ${rollback_rc}. Manual rollback command: bash scripts/deploy/deploy-snapshot-rollback.sh rollback --snapshot-id ${snapshot_id}"
                fi
            ...
            ok "Maintenance deploy завершён. Rollback snapshot сохранён: ${snapshot_id}"
            ;;
        vps1)
            [[ "$regen_configs" == "true" ]] && err "--regen-configs доступен только для полного деплоя"
            log "Запуск reprovision деплоя VPS1 (deploy-vps1.sh генерирует новые WG-ключи)..."
        ...
        split-tunneling)
            ...
            if [[ "$rollback" == "true" ]]; then
                ...
                log "Включение раздельного туннелирования (RU TLD + category-ru мимо VPN) на VPS1..."
