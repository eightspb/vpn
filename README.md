...
|----------|--------|-----------|
| IP-адрес VPS1 | `89.169.172.51` | Панель хостинга |
...
- macOS (основной сценарий) или Linux
- zsh/bash
- SSH-клиент
- [uv](https://docs.astral.sh/uv/) для локального Python-окружения админки
...
| 10.8.0.2 | VPS2 в туннеле (AdGuard Home DNS) |
...
ssh -i .ssh/ssh-key-1772056840349 slava@89.169.172.51 "sudo awg show awg1"
```

Вывод покажет каждый пир: публичный ключ, IP, время последнего соединения и трафик.

## DNS-фильтрация через AdGuard Home

| Компонент | Адрес | Назначение |
|---|---|---|
| DNS-сервер | `10.8.0.2:53` | Блокировка рекламных, трекинговых и malware-доменов для VPN-клиентов |
| Web UI | `http://10.8.0.2:3000` | Управление AdGuard Home, доступ только через VPN-интерфейс |

AdGuard Home ставится стандартным `bash manage.sh deploy` на VPS2. Клиентские конфиги используют DNS `10.8.0.2`, а на VPS1 DNS-запросы клиентов DNAT-ятся на VPS2.

Ограничение: DNS-фильтрация не является надёжным способом убрать video ads YouTube, потому что YouTube часто отдаёт рекламу и контент с одних и тех же доменов. Для YouTube используйте клиентские средства: браузерный блокировщик, AdGuard-приложение на устройстве или YouTube Premium.
...
```bash
# VPS1 (Яндекс Москва)
ssh -i .ssh/ssh-key-1772056840349 slava@89.169.172.51
...
bash manage.sh deploy \
  --vps1-ip 89.169.172.51 --vps1-user slava --vps1-key .ssh/ssh-key-1772056840349 \
  --vps2-ip 38.135.122.81 --vps2-key .ssh/ssh-key-1772056840349 \
  --adguard-pass "Strong-Password-123"
```

**Если при деплое пишет «Не удалось подключиться к VPS1»:** скрипт выведет последние строки вывода `ssh -v`. Проверьте: (1) доступность хоста: `ping 89.169.172.51` или `Test-NetConnection -ComputerName 89.169.172.51 -Port 22` в PowerShell; (2) в `.env` указан верный `VPS1_KEY` (путь к приватному ключу) и `VPS1_USER`; (3) на сервере в `~/.ssh/authorized_keys` добавлен публичный ключ этого пользователя; (4) SSH с ключом вручную: `ssh -i .ssh/ваш_ключ VPS1_USER@VPS1_IP`.
...
**Раздельное туннелирование российских TLD и `category-ru` на VPS1 (без перевыпуска клиентских конфигов):**
```bash
# Проверки без изменения серверов
bash tests/test-split-tunneling.sh
bash scripts/deploy/setup-split-tunneling.sh --dry-run

# Включить .ru/.рф/.su + category-ru/Ozon через VPS1, остальное через VPS2
...
обратно как `10.8.0.2`, чтобы старые клиентские DNS-настройки работали. Базовый
`/etc/dnsmasq.d/vpn.conf` складывает ответы для `.ru/.рф/.su` в `ipset ru_subnets`,
а `/etc/dnsmasq.d/vpn-ru-domains.conf` генерируется из `v2fly/domain-list-community`
`category-ru` с локальным Ozon seed (`ozon.com`, `ozonusercontent.com`,
`ozonglobalevents.com`, etc.). Поэтому российские сервисы на `.com`/CDN-доменах
тоже получают маршрут через основной интерфейс VPS1. Для DNS добавляется `INPUT`
allow на `10.9.0.1:53` и отдельное policy rule
`10.9.0.1 -> 10.9.0.0/24 lookup main`, потому что после DNAT запрос обслуживает
локальный `dnsmasq`, а его ответы иначе попадают под старое правило
`from 10.9.0.0/24 lookup 200`. Клиентские `AllowedIPs`, ключи, `awg0/awg1` и
...
> **Примечание:** `monitor-web.sh` запускает HTTP-сервер на `127.0.0.1:8080`.
> В текущем локальном сценарии (macOS/Linux) этого достаточно для открытия
> дашборда в браузере. Скрипт также автоматически определяет, доступен ли VPS1
> по внутреннему IP `10.9.0.1` (клиентский шлюз VPN).
...
Локальный и серверный контур админки теперь использует `uv` для подготовки Python-окружения. На VPS1 `redeploy-admin-vps1.sh` загружает свежий код, а `deploy-admin-vps1-remote.sh` автоматически ставит `uv`, пересобирает `scripts/admin/.venv`, обновляет systemd unit `vpn-admin` и перезапускает сервис.

#### Порядок обновления админки на сервере

```bash
# 1. Подтянуть код и проверить изменения локально
git pull --ff-only
bash tests/test-admin.sh

# 2. Обновить админку на VPS1
bash scripts/deploy/redeploy-admin-vps1.sh \
  --vps1-ip vpnrus.net \
  --vps1-user slava \
  --vps1-key .ssh/ssh-key-1772056840349

# 3. Проверить сервис и health на сервере
ssh -i .ssh/ssh-key-1772056840349 slava@vpnrus.net "sudo systemctl status vpn-admin --no-pager"
ssh -i .ssh/ssh-key-1772056840349 slava@vpnrus.net "curl -kfsS https://127.0.0.1:8443/api/health"
```

#### Что делает redeploy-скрипт

- Упаковывает текущий локальный snapshot репозитория
- Копирует его на VPS1 в `/tmp/vpn-deploy.tar.gz`
- Сохраняет `admin.db`, `certs/` и `.env`
- Разворачивает код в `/opt/vpn`
- Собирает `scripts/admin/.venv` через `uv`
- Перезапускает `vpn-admin` и выполняет smoke-check
- Заодно обновляет код `vpn-bot` на VPS1 и перезапускает его
...
Проверка изменений Фазы 3 (безопасность: eval убран, хардкод убран, add_phone_peer параметризован, AdGuard-only диагностика):
...
```bash
# macOS / Linux (требует Python 3 + curl)
...
При проблемах с VPN, DNS-фильтрацией, телефоном без интернета или дашбордом:
...
- Работу AdGuard Home (DNS порт 53, Web UI порт 3000)
