...
http_get() {
    local path="$1"
    curl -s -w "\n%{http_code}" -H "Authorization: Bearer $TOKEN" "${BASE_URL}${path}" 2>/dev/null
}
...
http_delete() {
    local path="$1"
    curl -s -w "\n%{http_code}" -X DELETE \
        -H "Authorization: Bearer $TOKEN" "${BASE_URL}${path}" 2>/dev/null
}
...
if [[ "$CODE" == "200" ]]; then
    ...
    fail "GET /api/auth/me via cookie session → $CODE (expected: 200)"
fi
...
CODE="$(echo "$RESP" | tail -1)"
if [[ "$CODE" == "401" ]]; then
    pass "GET /api/auth/me without token → 401"
else
    fail "GET /api/auth/me without token → $CODE (expected: 401)"
fi
...
if [[ "$CODE" == "200" ]]; then
    ...
    fail "GET /api/auth/me with token → $CODE (expected: 200)"
fi
...
# Create peer (will fail due to no SSH, but should return proper error)
...
if [[ "$CODE" == "201" ]]; then
    ...
    skip "Peer CRUD tests skipped (no VPN server)"
    PEER_ID=""
else
    fail "POST /api/peers → $CODE (expected: 201 or 500/502)"
    PEER_ID=""
fi

if [[ -n "${PEER_ID:-}" && "$PEER_ID" != "None" && "$PEER_ID" != "" ]]; then
    # Get peer
...
CODE="$(echo "$RESP" | tail -1)"
if [[ "$CODE" == "401" ]]; then
    pass "GET /api/auth/me after logout → 401 (token invalidated)"
else
